...
首次启动会自动创建 `database/vCampus.accdb` 及各模块 Access 表。同一条服务端命令会在同一个 Java 进程中同时开启校园服务 `8888` 和校园卡网关 `8889`，不需要再启动第二个服务端；看到两个端口的启动提示后保持终端运行。账号与各模块业务数据都会保存在服务器的同一 Access 数据库中。
...
最终演示账号（统一初始密码 `123456`）：

| 用这个账号测试什么 | 服务器中额外登记的资料或权限 | 一卡通号 | 密码 |
|---|---|---|---|
| 全系统管理 | 可以管理账号和所有业务模块 | `20260000` | `123456` |
| 学籍管理 | 可以维护学籍数据 | `20260001` | `123456` |
| 选课管理 | 可以维护选课数据 | `20260002` | `123456` |
| 图书馆管理 | 可以维护图书馆数据 | `20260003` | `123456` |
| 商店管理 | 可以维护商店数据 | `20260004` | `123456` |
| 医院管理 | 可以维护医院数据 | `20260005` | `123456` |
| 普通学生 | 6 名组员、周一至周九 | `20260006`—`20260020` | `123456` |
| 教师 | 8 名公共教师 | `20260021`—`20260028` | `123456` |
| 医生 | 10 名医院医生 | `20260029`—`20260038` | `123456` |

这些是公开的虚构测试账号，简单密码只用于联调。全局 `Role` 不区分学生、教师和医生；资格分别来自学籍、教师和医院业务表，并通过稳定的内部 `userId` 关联。

最终演示数据的完整清单、本地独立测试和 Radmin VPN 统一服务器操作见[最终演示数据库初始化数据清单](database/FINAL_DEMO_INITIAL_DATA.md)。安全重建命令统一列在本页第 2 节，正常启动服务器不会清空已有数据。

登录页不再展示测试账号清单，账号以本文档为准。校园服务首页保留修改密码与退出登录；进入模块后隐藏公共大标题和底部连接测试区，只保留一个返回箭头，优先返回模块内上一层，再返回校园服务。学籍查询仅在服务器确认本人有学生档案时填入本人学号，非学生账号保持空白。

界面只保留业务标题、字段、必要规则与操作结果，不常驻展示服务器鉴权、数据库来源、自动同步等实现说明。权限检查仍由服务器执行，并未因删除文字而取消。日期格式、退课与停用后果、医疗安全提示，以及专门帮助页面中的流程说明继续保留。

窗口布局以普通窗口（约 1280 × 800）为设计基准，同时适配小窗口与全屏：公共页面内容居中，最大宽度为 1280 个逻辑像素；卡片按可用宽度调整列数，学籍六张卡片始终等宽等高，其他卡片行高按内容决定；工具栏自动换行并保留操作空间。表单保持适合阅读的宽度与自然高度，内容超出窗口时纵向滚动；名单与表格仍使用剩余高度。系统缩放由 Swing 处理，不要求固定全屏使用。

模块页面统一使用“返回箭头 + 页面标题”的横向标题栏，右侧只显示当前姓名，不追加角色括号。进入内层页面后，标题和返回目标随当前页面切换；权限检查、充值等业务按钮仍保留。首页采用深蓝与青绿色、少量暖金点缀，业务页面保持浅色背景与简洁卡片，不改变接口、权限或数据库。

登录页采用居中的横向组合卡片：左侧为校园线稿与居中标题，右侧为登录表单。卡片最大为 1100 × 600 个逻辑像素，在小窗口内缩小、在全屏中保持紧凑；装饰图形由程序绘制，不依赖外部图片。

图书馆、商店和医院的模式选择页使用横向长条卡片，按行排列：左侧显示名称与功能摘要，右侧为进入按钮。图书馆与医院按三行均分可用高度；图书馆只有两个入口时占上面三分之二。高度不足时保留最小卡片高度并允许滚动。

学籍尚未查询学生时，“开具证明”和“查看毕业审核”按钮保持显示但置灰；查询到可操作的学生档案后启用，不绕过原有本人及管理员权限限制。

超级管理员在“用户管理 → 操作记录”可查看全部账号审计及各模块数据库写入，支持调整窗口、搜索、排序和复制完整详情，不限制最近 200 条。日志不保存密码、图片内容或病历正文，回滚的 DML 不会留下成功写入记录。详细范围见[数据库说明](database/README.md#统一数据库操作记录)。

### 图书馆完整演示

图书馆与其他模块统一使用 `database/vCampus.accdb`，不再为图书馆创建
`library-demo-01.accdb` 等独立数据库。需要从初始状态完整演示时，由服务器负责人按第 2 节执行一次安全重建；一轮演示期间始终使用同一数据库。重启服务器后检查预约、借阅和单册状态
是否保持不变。返回校园服务或重新进入图书馆只会重置界面导航，不会重置这些业务数据。

然后按以下顺序演示：

1. 使用普通学生 `20260006 / 123456` 登录，进入“图书馆 → 线上图书馆 → 我的图书馆”。
   当前借阅可看到 `Java编程思想 / SEU-B001-001`，历史借阅可看到
   `深入理解Java虚拟机 / SEU-B002-001`，我的预约可看到
   `数据结构（Java语言描述） / SEU-B003-001` 的“待取”状态。
   选中当前借阅可演示续借：未逾期、未续借且同书目无人预约时，从原到期日顺延 30 天。
...
4. 退出并使用图书馆管理员 `20260003 / 123456` 登录。模式选择页会把读者入口与
   “图书管理员工作台”分开；管理员仍可作为读者进入线上图书馆。该账号预置了
   `史记 / SEU-B005-002` 逾期记录，可演示逾期时禁止借书、预约和续借。
   随后进入管理员工作台，找到刚归还的单册，确认其状态为“待上架”并执行“确认上架”；
   书目维护页还可新增分类，再创建引用该分类的新书目。
5. 在管理员借阅查询以及学生“我的图书馆”中核对借阅、归还和预约状态。重启服务器后再次查询，
   状态应保持不变；已有数据库不会重新生成被取消或已完成的演示记录。
...
当前可复现：登录门禁、模块大厅、登出、PING/PONG、超级管理员账号维护与 CSV 批量导入、学生学籍查询与异动、选课与退课、图书检索预约与模拟借还、同进程独立端口的校园卡网关（Access 余额、充值、扣款与流水）、商店购物车及校园卡支付、医院患者号源查询与校园卡缴费、医院三模式入口，以及“医院管理员分类申请—超级管理员审核—账号关联/生成—申请记录交付账号—医生档案激活”链路。停止服务器时在服务器终端按 `Ctrl + C`。
...
3. 在服务器电脑运行原有的一条服务端命令。同一个进程会自动监听 8888 和 8889；在 Windows 防火墙提示时允许 Java 通信，无需另开校园卡服务端，也不要把端口映射到公网。
