...
这些是公开的虚构测试账号，统一简单密码仅用于联调，不得用于真实系统或复用个人密码。每个人只有一个账号。“患者”不是一种账号身份，所有已登录账号都可以进入患者模式。当前为了复用已有演示数据，`teacher001` 的基础身份仍是教师；它能进入医生模式，是因为医院医生名单中登记了它的 `userId`，不是因为教师自动等于医生。
