...
| 全系统管理 | 可以管理账号和所有业务模块 | `20260003` | `123456` |
| 学籍管理 | 可以维护学籍数据 | `20260004` | `123456` |
| 选课管理 | 可以维护选课数据 | `20260005` | `123456` |
| 图书馆管理 | 可以维护图书馆数据 | `20260006` | `123456` |
| 商店管理 | 可以维护商店数据 | `20260007` | `123456` |
| 医院管理 | 可以维护医院数据 | `20260008` | `123456` |

这些是公开的虚构测试账号，统一简单密码仅用于联调，不得用于真实系统或复用个人密码。一卡通号固定为“4 位年份 + 4 位流水号”，流水号范围为 `0001`—`9999`，每个人只有一个账号。“学生、教师、医生、患者”都不是全局登录角色，由对应子系统根据 `userId` 查询自己的业务资料。所有已登录账号都可以进入患者模式；`20260002` 能进入医生模式，是因为医院有效医生档案绑定了它的 `userId`。
...
批量导入文件使用 UTF-8 CSV，第一行固定为 `campusCardNumber,displayName`。每次最多
