...
各模块通过共享 `AccessDatabase.openConnection()` 写入自己的表，无需修改已有 Action 或 DTO。新增 `tblDatabaseAuditLog` 记录普通、预编译及批量语句的新增、修改、删除，日志与业务 DML 使用同一事务：提交一同保存，回滚一同撤销；日志写入失败时业务 DML 回滚。建表、改表和删表也记录结构事件，但受驱动事务规则影响，不能承诺与 DML 相同的原子性。

内容为时间、操作者 `userId`、一卡通号、姓名快照、业务 Action、请求编号、表名、写入类型和影响条数。不保存完整 SQL、绑定参数、密码、token、图片或医疗正文。未登录请求标记为匿名；非网络写入标记为系统操作。审计表自身不重复记录，查询和零行更新不增加日志。

`USER.ADMIN_LIST_AUDIT_LOGS` 仍只允许超级管理员调用，合并返回全部账号业务审计与数据库写入记录。一次业务可能涉及多张表，因此可能产生多条记录。客户端支持搜索、排序、可缩放窗口和可复制的完整详情。正常启动自动增加审计表，无需重建；日志从更新后的服务器开始积累，不倒补旧操作，也不监听直接打开 Access 文件产生的外部修改。
