...
| 表名 | 业务含义 | 主键 | 重要约束 |
|---|---|---|---|
| `tblCampusCard` | 虚拟校园卡余额 | `userId` | `campusCardNumber` 与登录账号相同且唯一 |
| `tblCampusCardLedger` | 充值/扣款/退款流水 | `txnId` | `(userId, merchant, reference, entryType)` 唯一，用于幂等 |
...
| `campusCardNumber` | Short Text(8) | 是 | | 一卡通号，与用户模块 `username` 相同 |
...
最终演示数据库为 39 个账号各写入 100.00 元。普通启动只为缺少校园卡的新账号补建卡片，不重置已有余额；完整账号与 `userId` 对照见 [`../FINAL_DEMO_INITIAL_DATA.md`](../FINAL_DEMO_INITIAL_DATA.md)。

## 6. 调用约定

- 商店下单继续使用共享 Access 事务同时完成余额、库存和订单更新；校园卡界面和其他费用仍走独立网关。
