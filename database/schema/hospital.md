...
| `doctorName` | Short Text(100) | 是 | 无 | 医院业务展示姓名，由入驻信息或虚构演示数据得到 |
...
- 医院可以先建立用于排班和挂号展示的虚构医生档案，此时 `userId = null`，该医生不能登录医生工作台。
- 医生入驻申请通过后，把唯一公共 `userId` 绑定到医生档案；一个账号最多绑定一名医生。
- 全局 `Role` 不包含医生；医生模式必须按当前 `userId` 找到 `active = Yes` 的医生档案。
- `doctorName` 是医院业务姓名，登录名、密码和全局显示名仍由用户模块拥有，医院模块不更新用户表。
...
真实重启验收只读访问本机 `database/vCampus.accdb`，创建可写测试副本；原库 SHA-256 保持不变。仅该副本加入测试排班和 `student001` 测试医生绑定。本机 `teacher002` 登录不可用，因此不作为本轮验收账号。
