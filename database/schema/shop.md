...
| `tblShopSeedVersion` | 官方种子目录升级版本 | `seedKey` 主键；`appliedAt` 时间；与迁移一同提交 |
...
官方商品目录按版本只补齐一次。旧库首次进入该版本时仅添加缺失的官方种子商品与照片，不覆盖已有商品、库存或照片，不删除已有旧商品，完成后登记版本。之后正常启动不再反复改写种子，保留管理员编辑。新库直接写入种子并登记版本；需要获得干净的最新官方初始清单时使用统一安全重建命令。
