...
医院申请分为两类。关联已有校园账号时，医院在提交阶段通过只读 `UserDirectory.findByCampusCardNumber(...)` 精确查询一卡通号并锁定其 `userId`；找不到或已禁用时拒绝提交。新建外来医生时不接收医院填写的一卡通号，超级管理员批准后才通过受限的 `AccountProvisioning.createGeneratedRegularAccount(...)` 创建账户：用户模块以“当前年份 + 当年最大流水号加一”生成唯一一卡通号，以初始密码 `123456` 创建 `Role.USER` 账号并返回新 `userId`。禁止根据姓名或碰巧重复的输入自动复用账户。
...
`20260000` 至 `20260007`，保留原 `userId`、显示名称、角色、管理范围和启停状态。由于开发期
