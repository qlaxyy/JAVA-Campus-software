...
- 当前密码只保存确定性 SHA-256 proof，不保存明文；带盐慢哈希仍是交付前待办。
- 会话连续空闲 30 分钟或创建满 8 小时后失效；暂不持久化，服务器重启后要求重新登录。
...
- UCanAccess 的 `currentRowMatches` 会把更新时正常跳过的不匹配行记录为警告；服务器仅屏蔽这一类驱动噪声，实际 SQL 异常仍会抛出并终止事务。
