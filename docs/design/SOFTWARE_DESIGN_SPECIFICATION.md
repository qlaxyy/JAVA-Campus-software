...
负责人：葛丰玮。当前已有商品查询与发布、购物车、校园卡充值与支付、个人订单取消退款以及商家成交查询。服务器始终使用会话 `userId` 归属余额和订单；登录一卡通号同时作为界面展示的卡号，不再生成第二套编号。商品分类、商品与库存、校园卡余额、服务器购物车、订单及订单明细均已迁移至 Access；支付时的余额扣减、库存扣减和订单创建由同一事务处理。
