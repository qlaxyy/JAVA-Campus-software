...
`CampusServer` 默认在所有可用网络接口上监听 8888 端口，因此同一可互通局域网中的其他电脑可使用服务器的 IPv4 地址连接。能否跨电脑连通还取决于 Windows 防火墙和路由器是否允许设备互访。服务器接收一个连接后，将处理任务提交到固定大小的工作线程池。每个连接当前只处理一个 Request 和一个 Response，然后关闭连接。
...
客户端程序通过启动参数接收服务器地址和端口，例如 `java -jar vcampus-client-0.1.0-SNAPSHOT.jar 192.168.1.23 8888`。自动测试只能证明监听方式、并发处理和会话隔离正确；最终验收还必须让服务器和客户端在两台真实电脑上通过局域网完成登录及至少一项业务操作。本课程项目的 Socket 协议没有 TLS，不应直接暴露到公网。
