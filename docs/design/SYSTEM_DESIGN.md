...
`SUPER_ADMIN` 的全系统管理能力来自 `Role`，不受上述单个子系统限制。教师资格单独保存在公共 `tblTeacherProfile` 中，由超级管理员维护；修改自己的密码时，客户端携带当前密码证明和新密码证明，服务器通过 token 确定被修改的账号，验证当前密码后保存新密码，并清除该账号的全部旧会话。
