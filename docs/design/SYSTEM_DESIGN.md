...
| `username` | 一卡通号（代码为兼容现有接口保留该字段名） |
| `displayName` | 页面显示名称 |
| `role` | `USER` 或 `SUPER_ADMIN` |
| `adminScopes` | 该账号可管理的子系统集合 |

当前会话只保存在用户服务器进程的内存中，不使用数据库。服务器同时记录会话创建时间和最后访问时间：连续 30 分钟没有有效请求，或单次登录累计达到 8 小时，token 自动失效。客户端下一次收到 `AUTH_REQUIRED` 时会清除本地会话并返回登录页。关闭客户端会丢失客户端会话；重启服务器也会清空全部 token。

超级管理员执行新增账号、批量导入、编辑、启停或重置密码时，用户服务器会向 Access 的 `tblUserAuditLog` 追加操作记录。记录包含操作者、操作、目标、结果和时间，不保存密码、密码 proof 或 token；超级管理员可在账号管理页点击“操作记录”查看全部记录。
...
| `ServerContext.teachers()` / `TeacherDirectory` | 用户模块 | 需要按 `userId` 查询有效教师或取得公共教师名单的服务器子系统 |
...
`SUPER_ADMIN` 的全系统管理能力来自 `Role`，不受上述单个子系统限制。教师资格单独保存在公共 `tblTeacherProfile` 中，由超级管理员维护；修改自己的密码时，客户端携带当前密码证明和新密码证明，服务器通过 token 确定被修改的账号，验证当前密码后保存新密码，并清除该账号的全部旧会话。Access 中实际保存每账号独立随机盐的 PBKDF2-HMAC-SHA256 结果和迭代次数，不保存可用于登录的新密码 proof；旧数据库账号在第一次成功登录后自动升级。
...
子系统管理员忘记密码时由超级管理员通过现有账号管理功能重置。唯一超级管理员忘记密码时不提供远程“找回密码”或万能密码；服务器维护者必须先停服，再在数据库所在电脑通过 `--reset-super-admin-password` 执行本地恢复。恢复程序只接受 `Role.SUPER_ADMIN` 账号，密码不进入命令行参数，结果写入 `tblUserAuditLog`。
...
医生新增不是医院管理员直接写入有效档案。申请分为两类：关联已有账号时，医院管理员明确填写已存在的一卡通号，服务器在提交阶段校验并锁定其 `userId`；新建外来医生时只填写姓名、科室和职称，不填写一卡通号。超级管理员批准外来医生后，用户模块按“当前年份 + 当年最大流水号加一”生成唯一一卡通号并创建 `Role.USER` 账号，医院模块随后以确定的 `userId` 激活医生档案。医院管理员可在“医生申请记录”中查看审核状态和最终一卡通号，并将一卡通号及初始密码交给医生；拒绝的申请不创建账号、不授予医生能力。

因此，总控负责让医院模块可靠地取得当前 `userId` 和管理范围，并提供只允许审批流程使用的内部账号开通接口；医院负责人负责申请、医生档案、医生资格查询以及医生业务权限。教师基础资格由用户模块的公共教师表统一提供；教师与教学班的任课关系、学生选课关系仍由选课模块负责。
