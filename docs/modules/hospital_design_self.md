...
> 文档状态：2026-09-09 已补充规则智能导诊实现；原“首诊/复诊二分法”由第 10 节的“诊疗阶段、检查结果回诊、复诊”模型取代。
>
...
- 输入：`TriageRequest`，包含 2～500 字的主要不适描述，以及用户主动勾选的“可能紧急”标志；勾选紧急时允许描述为空。
- 处理：服务器先执行高风险关键词拦截，再按本地关键词规则评分；候选范围来自当前启用且可挂号的具体科室，最多返回 3 个结果。
- 输出：`TriageResultView`。普通结果包含科室编号、名称、匹配程度和可解释理由；紧急结果只包含线下求助提示，不返回普通预约建议。
- 降级：没有明确专科线索时建议全科门诊；匹配不到可用科室时返回空建议并提示患者查看科室目录。
- 数据边界：症状描述和导诊结果不写入 Access，不进入健康档案、诊疗记录或管理员页面。
- 医疗边界：结果只是选科辅助，不提供疾病名称、治疗意见、处方或自动预约。后续如接入 LLM，只能负责自然语言抽取和追问，最终安全拦截、科室白名单与推荐约束仍由服务器规则负责。
