...
| 医院 | `01-需求基线.md` 4.6 | 医院模块设计、`database/schema/hospital.md` | [#4](https://github.com/qlaxyy/JAVA-Campus-software/issues/4) | PR #23、#24 | `HospitalServiceTest`、`HospitalSearchIntegrationTest` | 待补充 | 患者端号源查询及患者/医生/管理员三模式已在本地集成 |
...
| 系统结构 | `design/SYSTEM_DESIGN.md`、根目录 `README.md` | 三模块 POM、六模块扩展位置、架构 ADR | PR #7、#8 已合并；六模块进入第一轮并行开发 |
| 公共 Message | `design/SYSTEM_DESIGN.md` 的“子系统公共接口” | Request/Response、ActionRouter、PING、登录/会话、单元/集成测试 | 路由、连接和基础会话契约已实现，业务契约由各模块设计 |
| 网络与多线程 | `design/SYSTEM_DESIGN.md` 的“登录与会话” | Socket、固定线程池、超时、PING/PONG 测试 | 最小链路已验证，完整策略待 ADR |
| 数据库 | `design/SYSTEM_DESIGN.md`、`database/schema/` | E-R 图、数据字典、建库脚本、连接验证 | 未开始 |
| GUI 规范 | 根目录 `README.md`、模块 Epic | 主导航原型、六模块入口、界面截图 | 统一导航和占位页已实现，业务页面待开发 |
...
| ADR-0009 管理员权限模型 | 通过，基础实现待合并 | 三种基础角色、附加 AdminScope、SessionInfo、五个子系统管理员、模块内模式、UML/流程图和相关测试 | 超级管理员账号管理页面、各模块管理 Action、DAO 与真实数据维护界面 |
