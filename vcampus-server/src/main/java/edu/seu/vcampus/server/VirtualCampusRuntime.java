...
public final class VirtualCampusRuntime implements AutoCloseable {
    ...
    private final DatabaseProcessLock databaseLock;

    private VirtualCampusRuntime(
            CampusServer campusServer,
            CampusServer cardServer,
            DatabaseProcessLock databaseLock) {
    ...
    public static VirtualCampusRuntime start(
        ...
        DatabaseProcessLock databaseLock = DatabaseProcessLock.acquire(databasePath);
        CampusServer cardServer = null;
        try {
            ...
            cardServer = new CampusServer(
                    cardPort, ServerModules.createCardRouter(wallet, context));
            cardServer.start();
            CampusCardWallet gateway =
                    new CampusCardTcpClient("127.0.0.1", cardServer.getPort());
            CampusServer campusServer = new CampusServer(
                    ...
                    ServerModules.createAccessCampusRouter(
                            ...
                            gateway));
            campusServer.start();
            return new VirtualCampusRuntime(campusServer, cardServer, databaseLock);
    ...
    public void close() {
        try {
            ...
            try {
                ...
                throw new IllegalStateException("Cannot release database process lock.", exception);
            }
        }
    }
}
